...
void ImGui::ShowDemoWindow(bool* p_open) {
  ...
  if (ImGui::CollapsingHeader("Widgets")) {
    if (ImGui::TreeNode("Basic")) {
      ...
      // framed elements (otherwise a Text+SameLine+Button sequence will have
      // the text a little too high by default)
...

      {
        ...
        ImGui::SameLine();
        ShowHelpMarker(
            "Refer to the \"Combo\" section below for an explanation of the "
            "full BeginCombo/EndCombo API, and demonstration of various "
            "flags.\n");
      }
        ...
        for (int n = 0; n < IM_ARRAYSIZE(items); n++) {
          ...
          if (is_selected)
            ImGui::SetItemDefaultFocus();  // Set the initial focus when opening
                                           // the combo (scrolling + for
                                           // keyboard navigation support in the
...
// signed/int/long long and float/double To avoid polluting the public API with
// all possible combinations, we use the ImGuiDataType enum to pass the type,
...
// natural type maximum, hence the /2 below.
  ...
  if (ImGui::CollapsingHeader("Popups & Modal windows")) {
    ...
    // Those three properties are intimately connected. The library needs to
    ...
    if (ImGui::TreeNode("Context menus")) {
      ...
      // will use the last item ID as the popup ID. In addition here, we want to
