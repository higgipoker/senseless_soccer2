...
// For InputTextEx()
static bool InputTextFilterCharacter(int* p_char, ImGuiInputTextFlags flags,
...
static void DataTypeApplyOp(ImGuiDataType data_type, int op, void* output,
  ...
  switch (data_type) {
    ...
    case ImGuiDataType_U32:
      if (op == '+')
        *(int*)output = *(const int*)arg1 + *(const ImU32*)arg2;
      else if (op == '-')
        *(int*)output = *(const int*)arg1 - *(const ImU32*)arg2;
      return;
...
const char* ImParseFormatFindEnd(const char* fmt) {
  ...
  if (fmt[0] != '%') return fmt;
  const int ignored_uppercase_mask = (1 << ('I' - 'A')) | (1 << ('L' - 'A'));
  const int ignored_lowercase_mask = (1 << ('h' - 'a')) | (1 << ('j' - 'a')) |
                                     (1 << ('l' - 'a')) | (1 << ('t' - 'a')) |
                                     (1 << ('w' - 'a')) | (1 << ('z' - 'a'));
    ...
    if (io.InputCharacters[0]) {
      ...
      if (!ignore_inputs && is_editable && !user_nav_input_start)
        for (int n = 0;
          ...
          int c = (int)io.InputCharacters[n];
  ...
  if (g.ActiveId == id && !g.ActiveIdIsJustActivated && !clear_active_id) {
    ...
    if (IsKeyPressedMap(ImGuiKey_LeftArrow)) {
      ...
      if (const char* clipboard = GetClipboardText()) {
        ...
        const unsigned int clipboard_len = (int)strlen(clipboard);
        ImWchar* clipboard_filtered =
            (ImWchar*)MemAlloc((clipboard_len + 1) * sizeof(ImWchar));
        unsigned int clipboard_filtered_len = 0;
...
bool ImGui::ColorEdit4(const char* label, float col[4],
  ...
  if ((flags & (ImGuiColorEditFlags_RGB | ImGuiColorEditFlags_HSV)) != 0 &&
    ...
    if (InputText("##Text", buf, IM_ARRAYSIZE(buf),
      ...
      i[0] = i[1] = i[2] = i[3] = 0;
      if (alpha)
        sscanf(p, "%02X%02X%02X%02X", (int*)&i[0], (int*)&i[1], (int*)&i[2],
               (int*)&i[3]);  // Treat at unsigned (%X is unsigned)
      else
        sscanf(p, "%02X%02X%02X", (int*)&i[0], (int*)&i[1], (int*)&i[2]);
    }
    if (!(flags & ImGuiColorEditFlags_NoOptions))
      OpenPopupOnItemClick("context");
    PopItemWidth();
